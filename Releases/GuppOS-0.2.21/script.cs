...
// 
// All configuration is done in the custom data of the programable block, or in the blocks of LCD screens.
// Designed for the Sigma Draconis Expanse 2 server
// 
// DO NOT TOUCH ANYTHING BELOW
// ----------------------
static string _Version="0.2.21",ignoreTag;static bool logAll,isBooting;TimeSpan timeSinceLooped;public
 Program(){
...
EchoLog(){for(int i=0;i<=5;i++){foreach(KeyValuePair<string,logItem>item in logItems){if(item.Value.priority==i){Echo("("+item.
Value.life+") [color=#ffff9600]"+item.Value.category+":[/color] "+item.Value.content);item.Value.life++;if(item.Value.life==
item.Value.maxLife){logItems.Remove(item.Key);}}}}}void EchoHeader(int incr){string headerStart;string headerEnd;switch(incr
){case 1:headerStart="[color=#ff00ff00]-==[/color]";headerEnd="[color=#ff00ff00]==-[/color]";break;case 2:headerStart=
"[color=#ff00ff00]=-=[/color]";headerEnd="[color=#ff00ff00]=-=[/color]";break;case 3:headerStart="[color=#ff00ff00]==-[/color]";headerEnd=
"[color=#ff00ff00]-==[/color]";break;default:headerStart="[color=#ff00ff00]===[/color]";headerEnd="[color=#ff00ff00]===[/color]";break;}Echo(
headerStart+"GuppOS, Version: "+_Version+headerEnd);}string[]defaultConfigSections=new string[]{"GuppOS.General",
"GuppOS.PowerManager","GuppOS.Tanks","GuppOS.Propulsion","GuppOS.LifeSupport","GuppOS.Lighting","GuppOS.Doors","GuppOS.Connectors"};List<
string>loadedSections=new List<string>();MyIni _config=new MyIni();bool LoadConfig(){string customData=Me.CustomData;
MyIniParseResult result;if(!_config.TryParse(customData,out result)){Echo("Could not parse custom data.\n-"+result.ToString());logItems[
"CfE01"]=new logItem{category="Config",content="Could not parse custom data.\n-"+result.ToString(),priority=0,maxLife=-1};
return false;}List<string>sections=new List<string>();_config.GetSections(sections);foreach(string section in sections){Echo(
"Reading section: "+section);try{switch(section){case"GuppOS.General":logAll=_config.Get(section,"LogAll").ToBoolean();gOSLCDTag=_config.
Get(section,"LCDTag").ToString().Trim();ignoreTag=_config.Get(section,"IgnoreTag").ToString().Trim();break;case
"GuppOS.PowerManager":powerManagerEnabled=_config.Get(section,"PowerManagerEnabled").ToBoolean();mainBatteryGroupName=_config.Get(section,
"MainBatteryGroup").ToString().Trim();backupBatteryGroupName=_config.Get(section,"BackupBatteryGroup").ToString().Trim();
mainReactorGroupName=_config.Get(section,"MainReactorGroup").ToString().Trim();lowPowerPercent=_config.Get(section,"LowPowerLevel").ToSingle
();highPowerPercent=_config.Get(section,"HighPowerLevel").ToSingle();break;case"GuppOS.Tanks":mainFuelTankGroupName=
_config.Get(section,"MainFuelTankGroup").ToString().Trim();mainAirTankGroupName=_config.Get(section,"MainAirTankGroup").
ToString().Trim();break;case"GuppOS.Propulsion":mainDriveGroupName=_config.Get(section,"EpsteinDriveGroup").ToString().Trim();
rcsGroupName=_config.Get(section,"RCSGroup").ToString().Trim();gyroGroupName=_config.Get(section,"GyroGroup").ToString().Trim();
break;case"GuppOS.LifeSupport":ventGroupName=_config.Get(section,"VentGroup").ToString().Trim();gravGenGroupName=_config.Get(
section,"GravGenGroup").ToString().Trim();break;case"GuppOS.Lighting":lightGroupName=_config.Get(section,"LightGroup").ToString
().Trim();navLightTag=_config.Get(section,"NavLightTag").ToString().Trim();break;case"GuppOS.Doors":doorManagerEnabled=
_config.Get(section,"DoorManagerEnabled").ToBoolean();doorGroupName=_config.Get(section,"DoorGroup").ToString().Trim();
doorOpenLength=_config.Get(section,"DoorOpenLength").ToInt16();break;case"GuppOS.Connectors":connectorGroupName=_config.Get(section,
"ConnectorGroup").ToString().Trim();break;default:break;}}catch(Exception exc){Echo(exc+"\nCould not parse config section: "+section+
