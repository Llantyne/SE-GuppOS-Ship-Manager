...
// ----------------------
static string _Version="0.2.22",ignoreTag;static bool logAll,isBooting;TimeSpan timeSinceLooped;public
 Program(){
isBooting=true;Echo("Booting GuppOS, Version: "+_Version+". . . ");Echo("Loading custom data config. . . ");if(!LoadConfig()){
Echo("Config load failed, applying default config.");LoadDefaults();}Echo("Custom data config loaded.");Echo(
"Initializing block lists. . . ");InitBlocks();LoadBlocks(1);LoadBlocks(2);LoadBlocks(3);Echo("Block lists initialized.");FixPosture("Standby");loopVer=
1;Runtime.UpdateFrequency=UpdateFrequency.Update100;Echo("Boot complete.");isBooting=false;}public void
 Save
(){}public void
 Main
...
PowerManager();DoorManager();RefreshLCDs();if(loopVer<3){loopVer++;}else{loopVer=1;}}static Dictionary<string,logItem>logItems=new
Dictionary<string,logItem>();class logItem{public string category="Misc",content="";public int priority=3,maxLife=5,life=0;}void
EchoLog(){List<string>toRemove=new List<string>();for(int i=0;i<=5;i++){foreach(KeyValuePair<string,logItem>item in logItems){
if(item.Value.priority==i){Echo("("+item.Value.life+") [color=#ffff9600]"+item.Value.category+":[/color] "+item.Value.
content);item.Value.life++;if(item.Value.life==item.Value.maxLife){toRemove.Add(item.Key);}}}foreach(string thing in toRemove){
logItems.Remove(thing);}}}void EchoHeader(int incr){string headerStart;string headerEnd;switch(incr){case 1:headerStart=
"[color=#ff00ff00]-==[/color]";headerEnd="[color=#ff00ff00]==-[/color]";break;case 2:headerStart="[color=#ff00ff00]=-=[/color]";headerEnd=
"[color=#ff00ff00]=-=[/color]";break;case 3:headerStart="[color=#ff00ff00]==-[/color]";headerEnd="[color=#ff00ff00]-==[/color]";break;default:
headerStart="[color=#ff00ff00]===[/color]";headerEnd="[color=#ff00ff00]===[/color]";break;}Echo(headerStart+"GuppOS, Version: "+
_Version+headerEnd);}string[]defaultConfigSections=new string[]{"GuppOS.General","GuppOS.PowerManager","GuppOS.Tanks",
"GuppOS.Propulsion","GuppOS.LifeSupport","GuppOS.Lighting","GuppOS.Doors","GuppOS.Connectors"};List<string>loadedSections=new List<string>(
);MyIni _config=new MyIni();bool LoadConfig(){string customData=Me.CustomData;MyIniParseResult result;if(!_config.
TryParse(customData,out result)){Echo("Could not parse custom data.\n-"+result.ToString());logItems["CfE01"]=new logItem{
category="Config",content="Could not parse custom data.\n-"+result.ToString(),priority=0,maxLife=-1};return false;}List<string>
sections=new List<string>();_config.GetSections(sections);foreach(string section in sections){Echo("Reading section: "+section);
try{switch(section){case"GuppOS.General":logAll=_config.Get(section,"LogAll").ToBoolean();gOSLCDTag=_config.Get(section,
"LCDTag").ToString().Trim();ignoreTag=_config.Get(section,"IgnoreTag").ToString().Trim();break;case"GuppOS.PowerManager":
powerManagerEnabled=_config.Get(section,"PowerManagerEnabled").ToBoolean();mainBatteryGroupName=_config.Get(section,"MainBatteryGroup").
ToString().Trim();backupBatteryGroupName=_config.Get(section,"BackupBatteryGroup").ToString().Trim();mainReactorGroupName=
_config.Get(section,"MainReactorGroup").ToString().Trim();lowPowerPercent=_config.Get(section,"LowPowerLevel").ToSingle();
highPowerPercent=_config.Get(section,"HighPowerLevel").ToSingle();break;case"GuppOS.Tanks":mainFuelTankGroupName=_config.Get(section,
"MainFuelTankGroup").ToString().Trim();mainAirTankGroupName=_config.Get(section,"MainAirTankGroup").ToString().Trim();break;case
"GuppOS.Propulsion":mainDriveGroupName=_config.Get(section,"EpsteinDriveGroup").ToString().Trim();rcsGroupName=_config.Get(section,
"RCSGroup").ToString().Trim();gyroGroupName=_config.Get(section,"GyroGroup").ToString().Trim();break;case"GuppOS.LifeSupport":
ventGroupName=_config.Get(section,"VentGroup").ToString().Trim();gravGenGroupName=_config.Get(section,"GravGenGroup").ToString().Trim
();break;case"GuppOS.Lighting":lightGroupName=_config.Get(section,"LightGroup").ToString().Trim();navLightTag=_config.Get
(section,"NavLightTag").ToString().Trim();break;case"GuppOS.Doors":doorManagerEnabled=_config.Get(section,
"DoorManagerEnabled").ToBoolean();doorGroupName=_config.Get(section,"DoorGroup").ToString().Trim();doorOpenLength=_config.Get(section,
"DoorOpenLength").ToInt16();break;case"GuppOS.Connectors":connectorGroupName=_config.Get(section,"ConnectorGroup").ToString().Trim();
break;default:break;}}catch(Exception exc){Echo(exc+"\nCould not parse config section: "+section+
"\n-Applying default values.");LoadDefaultSection(section);logItems["CfE02:"+section]=new logItem{category="Config",content="Config section "+section
+" failed to parse; "+exc+"\n-Default values loaded for section.",priority=0,maxLife=-1};}if(section.Contains(
"GuppOS.Posture.")){string newPostureName=section.Substring(15);try{availablePostures.Add(newPostureName,BuildPosture(_config,section));}
catch(Exception exc){Echo(exc+"\n-Could not build posture "+newPostureName);logItems["CfE03:"+section]=new logItem{category=
"Config",content="Posture "+newPostureName+" failed to build; "+exc,priority=0,maxLife=-1};}}loadedSections.Add(section);}
foreach(string sec in defaultConfigSections){if(!loadedSections.Contains(sec)){Echo("Could not find config section: "+sec+
"\n-Applying default values.");LoadDefaultSection(sec);logItems["CfE04:"+sec]=new logItem{category="Config",content="Could not find config section: "
+sec+"\n-Default values loaded for section.",priority=0,maxLife=-1};}}logItems["CfS01"]=new logItem{category="Config",
content="Custom config applied.",priority=1,maxLife=-1};if(availablePostures.Count<=0){Echo(
"No custom postures found. Loading defaults.");availablePostures=BuildDefaultPostures();logItems["CfS02"]=new logItem{category="Config",content=
"Default postures applied",priority=1,maxLife=-1};}else{logItems["CfS02"]=new logItem{category="Config",content="Custom postures applied",priority
=1,maxLife=-1};}BuildBootPosture();return true;}void LoadDefaults(){foreach(string sec in defaultConfigSections){
LoadDefaultSection(sec);}availablePostures=BuildDefaultPostures();logItems["CfS01"]=new logItem{category="Config",content=
"Default config applied.",priority=1,maxLife=-1};logItems["CfS02"]=new logItem{category="Config",content="Default postures applied",priority=1,
maxLife=-1};}void LoadDefaultSection(string sect){switch(sect){case"GuppOS.General":logAll=true;gOSLCDTag="<GOSLCD>";ignoreTag=
"<I>";break;case"GuppOS.PowerManager":powerManagerEnabled=true;mainBatteryGroupName="Main Batteries";backupBatteryGroupName=
"Backup Batteries";mainReactorGroupName="ALL";lowPowerPercent=25;highPowerPercent=90;break;case"GuppOS.Tanks":mainFuelTankGroupName=
"Fuel Tanks";mainAirTankGroupName="Air Tanks";break;case"GuppOS.Propulsion":mainDriveGroupName="Main Drives";rcsGroupName=
"RCS Drives";gyroGroupName="Gyros";break;case"GuppOS.LifeSupport":ventGroupName="ALL";gravGenGroupName="ALL";break;case
"GuppOS.Lighting":lightGroupName="ALL";navLightTag="*NAV*";break;case"GuppOS.Doors":doorManagerEnabled=true;doorGroupName="ALL";
doorOpenLength=6;break;case"GuppOS.Connectors":connectorGroupName="ALL";break;}}static string connectorGroupName;ConnectorGroup
mainConnectors;class ConnectorGroup:BlockGroup<IMyShipConnector>{public ConnectorGroup(Program program,string name):base(program,name,
new string[]{"Co","Connectors"}){}public void SetConTag(string conTag,bool enabled){try{foreach(IMyShipConnector connector
in thingList){if(connector.CustomName.Contains(conTag)){connector.Enabled=enabled;}}}catch(Exception exc){logItems[logID[0
]+"E04:"+groupName]=new logItem{category=logID[1],content="Error when setting connector tag status of "+groupName+" -"+
exc,priority=1,maxLife=3};}}}static bool doorManagerEnabled;static string doorGroupName;static int doorOpenLength;class
SingleDoor{public string doorName;public IMyDoor doorBlock;public int timeOpen=0;public bool isLocked;public void SetDoorLock(bool
unlockDoor){if(unlockDoor&&isLocked){doorBlock.ApplyAction("AnyoneCanUse");isLocked=!isLocked;}if(!unlockDoor&&!isLocked){
doorBlock.ApplyAction("AnyoneCanUse");isLocked=!isLocked;}}}SDoorGroup mainSDoors;class SDoorGroup:ThingGroup<SingleDoor>{
protected Program program;public SDoorGroup(Program program,string name):base(name,new string[]{"SD","SingleDoors"}){this.program
=program;}public int sDoorsOpen=0,sDoorsFunc=0;public override void LoadGroup(bool managerStatus=false,bool hardLoad=
false){List<IMyDoor>doors=new List<IMyDoor>();if(groupName=="ALL"){program.GridTerminalSystem.GetBlocksOfType(doors,door=>
door.IsSameConstructAs(program.Me)&&!door.CustomName.Contains(ignoreTag));}else{IMyBlockGroup myBlockGroup=program.
GridTerminalSystem.GetBlockGroupWithName(groupName);if(myBlockGroup==null){logItems[logID[0]+"E01:"+groupName]=new logItem{category=logID[
1],content=groupName+" not found.",priority=1,maxLife=3};return;}myBlockGroup.GetBlocksOfType(doors,door=>door.
IsSameConstructAs(program.Me)&&!door.CustomName.Contains(ignoreTag));}List<SingleDoor>singleDoors=new List<SingleDoor>();if(thingList.
Count>0&&!hardLoad){singleDoors.AddList(thingList);}foreach(IMyDoor door in doors){if(singleDoors.Exists(d=>d.doorName==door.
CustomName)){continue;}singleDoors.Add(new SingleDoor{doorName=door.CustomName,doorBlock=door,isLocked=!program.IfUnlocked(door)})
;}thingList=singleDoors;ReadSDoors();if(logAll)logItems[logID[0]+"E01:"+groupName]=new logItem{category=logID[1],content=
"Single doors loaded.",priority=5,maxLife=1};}public void ReadSDoors(){try{sDoorsOpen=0;sDoorsFunc=0;foreach(SingleDoor sDoor in thingList){if
(sDoor.doorBlock.IsWorking){sDoorsFunc++;}if(sDoor.doorBlock.OpenRatio>0){sDoorsOpen++;}}}catch(Exception exc){logItems[
logID[0]+"E02:"+groupName]=new logItem{category=logID[1],content="Error when reading "+groupName+" -"+exc,priority=1,maxLife=
3};}}public void ActionSDoor(SingleDoor sDoor){try{if(sDoor.doorBlock.OpenRatio>0){if(sDoor.timeOpen>=doorOpenLength){
sDoor.doorBlock.CloseDoor();sDoor.timeOpen=0;}else{sDoor.timeOpen++;}}}catch(Exception exc){logItems[logID[0]+"E03:"+
groupName]=new logItem{category=logID[1],content="Error when actioning door of "+groupName+" -"+exc,priority=1,maxLife=3};}}
public void SetDoorTagLock(string doorTag,bool unlockDoors){try{foreach(var sDoor in thingList){if(sDoor.doorName.Contains(
doorTag)){sDoor.SetDoorLock(unlockDoors);}}if(logAll)logItems[logID[0]+"E01:"+groupName]=new logItem{category=logID[1],content=
"Doors with tag "+doorTag+" lock set to "+!unlockDoors,priority=4,maxLife=1};}catch(Exception exc){logItems[logID[0]+"E04:"+groupName]=
new logItem{category=logID[1],content="Error when setting door tag lock in "+groupName+" -"+exc,priority=1,maxLife=3};}}}
void DoorManager(){if(!doorManagerEnabled){return;}if(mainSDoors.thingList==null){logItems["DME01"]=new logItem{category=
"PowerManager",content="Block group(s) invalid, disabling Power Manager.",priority=1,maxLife=3};powerManagerEnabled=false;return;}
foreach(SingleDoor sDoor in mainSDoors.thingList){mainSDoors.ActionSDoor(sDoor);}if(logAll)logItems["DML01"]=new logItem{
category="DoorManager",content="Single doors actioned.",priority=5,maxLife=1};}bool IfUnlocked(IMyDoor door){ITerminalAction
action=door.GetActionWithName("AnyoneCanUse");StringBuilder status=new StringBuilder();action.WriteValue(door,status);return
status.ToString()=="On";}class ThingGroup<thingType>where thingType:class{public ThingGroup(string name,string[]iD){groupName=
name;logID=iD;}public string groupName;public string[]logID;public List<thingType>thingList=new List<thingType>();public
virtual void LoadGroup(bool managerStatus=false,bool hardLoad=false){}}class BlockGroup<blockType>:ThingGroup<blockType>where
blockType:class,IMyFunctionalBlock{protected Program program;public BlockGroup(Program program,string name,string[]iD):base(name,
iD){this.program=program;groupName=name;logID=iD;}public bool?groupEnabled=null;public override void LoadGroup(bool
managerStatus=false,bool hardLoad=false){List<blockType>blocks=new List<blockType>();if(groupName=="ALL"){program.GridTerminalSystem.
GetBlocksOfType(blocks,b=>b.IsSameConstructAs(program.Me)&&!b.CustomName.Contains(ignoreTag));}else{IMyBlockGroup myBlockGroup=program.
GridTerminalSystem.GetBlockGroupWithName(groupName);if(myBlockGroup==null){logItems[logID[0]+"E01:"+groupName]=new logItem{category=logID[
1],content=groupName+" not found.",priority=1,maxLife=3};return;}myBlockGroup.GetBlocksOfType(blocks,b=>b.
IsSameConstructAs(program.Me)&&!b.CustomName.Contains(ignoreTag));}thingList=blocks;ReadBlocks();if(logAll)logItems[logID[0]+"L01:"+
groupName]=new logItem{category=logID[1],content=groupName+" loaded.",priority=5,maxLife=1};}public void ReadBlocks(){try{
ReadStep();}catch(Exception exc){logItems[logID[0]+"E02:"+groupName]=new logItem{category=logID[1],content="Error when reading "
+groupName+" -"+exc,priority=1,maxLife=3};}}public virtual void ReadStep(){}public void SetGroupEnabled(bool enable){try{
if(enable!=groupEnabled){foreach(blockType drive in thingList){drive.Enabled=enable;}groupEnabled=enable;}}catch(Exception
exc){logItems[logID[0]+"E03:"+groupName]=new logItem{category=logID[1],content="Error when enabling/disabling "+groupName+
" -"+exc,priority=1,maxLife=3};}}}void InitBlocks(){mainBatteries=new BatteryGroup(this,mainBatteryGroupName);
backupBatteries=new BatteryGroup(this,backupBatteryGroupName);mainReactors=new ReactorGroup(this,mainReactorGroupName);allGOSLCDs=
LoadLCDs();mainDrives=new DriveGroup(this,mainDriveGroupName);rcsThrusters=new DriveGroup(this,rcsGroupName);mainGyros=new
GyroGroup(this,gyroGroupName);mainLights=new LightGroup(this,lightGroupName);mainFuelTanks=new GasTankGroup(this,
mainFuelTankGroupName);mainAirTanks=new GasTankGroup(this,mainAirTankGroupName);mainSDoors=new SDoorGroup(this,doorGroupName);mainAirVents=
new VentGroup(this,ventGroupName);mainGravGens=new GravGenGroup(this,gravGenGroupName);mainConnectors=new ConnectorGroup(
this,connectorGroupName);}void LoadBlocks(int sec){switch(sec){case 1:mainBatteries.LoadGroup(powerManagerEnabled);
backupBatteries.LoadGroup(powerManagerEnabled);mainReactors.LoadGroup(powerManagerEnabled);allGOSLCDs=LoadLCDs();mainDrives.LoadGroup()
;rcsThrusters.LoadGroup();mainGyros.LoadGroup();break;case 2:mainLights.LoadGroup();mainFuelTanks.LoadGroup();
mainAirTanks.LoadGroup();break;case 3:mainSDoors.LoadGroup(doorManagerEnabled);mainAirVents.LoadGroup();mainGravGens.LoadGroup();
mainConnectors.LoadGroup();break;default:break;}}static string gOSLCDTag,basicFont="Monospace";static Color titleColour=new Color(255,
255,255,255),basicColour=new Color(255,255,255,255),postureColour=new Color(255,128,192,255),enabledColour=new Color(0,255,
0,255),disabledColour=new Color(255,0,0,255),onlineColour=new Color(128,255,128,255),offlineColour=new Color(255,128,128,
255),goodColour=new Color(0,255,128,255),cautionColour=new Color(255,128,64,255),mildCautionColour=new Color(255,255,128,
255),autoModeColour=new Color(0,255,64,255),rechargeModeColour=new Color(255,255,0,255),dischargeModeColour=new Color(0,255
,255,255),defModeColour=new Color(0,255,64,255),stockpileModeColour=new Color(128,255,255,255);List<GOSLCD>allGOSLCDs;
class GOSLCD{public string lcdType="";public IMyTextSurface lcdScreen;public RectangleF drawableSurface;public float
fontScale=1.0f;public float[]margins;public void AdjustForMargins(){drawableSurface.Y+=margins[0];drawableSurface.Height-=margins
[0];drawableSurface.Height-=margins[1];drawableSurface.X+=margins[2];drawableSurface.Width-=margins[2];drawableSurface.
Width-=margins[3];}}MyIni _LCDConfig=new MyIni();List<GOSLCD>LoadLCDs(){List<GOSLCD>gLCDs=new List<GOSLCD>();List<
IMyTerminalBlock>lcdBlocks=new List<IMyTerminalBlock>();GridTerminalSystem.GetBlocksOfType(lcdBlocks,block=>block.CustomName.Contains(
gOSLCDTag)&&!block.CustomName.Contains(ignoreTag));foreach(IMyTerminalBlock block in lcdBlocks){string lcdCustomData=block.
CustomData;IMyTextSurfaceProvider lcdProvider=block as IMyTextSurfaceProvider;MyIniParseResult lcdResult;if(lcdProvider!=null&&
lcdProvider.SurfaceCount>0){if(!_LCDConfig.TryParse(lcdCustomData,out lcdResult)){logItems["LCDE02"]=new logItem{category="LCDs",
content="Could not parse custom data for "+block.CustomName+" -"+lcdResult.ToString(),priority=1,maxLife=3};continue;}if(
_LCDConfig.ContainsSection("GuppOS.LCD")){int screenIndex;List<MyIniKey>sectKeys=new List<MyIniKey>();_LCDConfig.GetKeys(
"GuppOS.LCD",sectKeys);foreach(MyIniKey key in sectKeys){if(int.TryParse(key.Name.Trim().Substring(4),out screenIndex)){try{string[]
LCDSetting=_LCDConfig.Get(key).ToString().Split(':');if(LCDSetting.Length<3){throw new Exception("Invalid config format for "+key.
ToString());}string[]LCDMargins=LCDSetting[2].Split(',');if(LCDMargins.Length<4){throw new Exception(
"Invalid config format for "+key.ToString());}GOSLCD newLCD=new GOSLCD{lcdType=LCDSetting[0].Trim(),lcdScreen=lcdProvider.GetSurface(screenIndex),
fontScale=Convert.ToSingle(LCDSetting[1].Trim()),margins=new float[]{Convert.ToSingle(LCDMargins[0].Trim()),Convert.ToSingle(
LCDMargins[1].Trim()),Convert.ToSingle(LCDMargins[2].Trim()),Convert.ToSingle(LCDMargins[3].Trim())}};newLCD.drawableSurface=new
RectangleF((newLCD.lcdScreen.TextureSize-newLCD.lcdScreen.SurfaceSize)/2f,newLCD.lcdScreen.SurfaceSize);newLCD.AdjustForMargins();
gLCDs.Add(newLCD);}catch(Exception exc){logItems["LCDE06"]=new logItem{category="LCDs",content=
"Error parsing custom data for "+block.CustomName+": "+exc,priority=1,maxLife=3};continue;}}else{logItems["LCDE05"]=new logItem{category="LCDs",content=
"Error parsing screen index in "+block.CustomName+" config line "+key,priority=1,maxLife=3};continue;}}}else{logItems["LCDE04"]=new logItem{category=
"LCDs",content=block.CustomName+" missing LCD config data.",priority=1,maxLife=3};continue;}}else{logItems["LCDE01"]=new
logItem{category="LCDs",content=block.CustomName+" does not have LCDs",priority=1,maxLife=3};continue;}}if(logAll)logItems[
"LCDL01"]=new logItem{category="LCDs",content=gLCDs.Count+" GuppOS LCDs loaded.",priority=5,maxLife=2};return gLCDs;}void
RefreshLCDs(){foreach(GOSLCD lcd in allGOSLCDs){switch(lcd.lcdType){case"Log":DrawLogLCD(lcd);break;case"Info":DrawInfoLCD(lcd);
break;default:break;}}}void DrawLogLCD(GOSLCD lcd){lcd.lcdScreen.ContentType=ContentType.SCRIPT;lcd.lcdScreen.Script="";int
logLineMaxChar=(int)Math.Floor(lcd.drawableSurface.Width/(19.4*0.8*lcd.fontScale));using(var frame=lcd.lcdScreen.DrawFrame()){float
logLineY=0;frame.Add(new MySprite{Type=SpriteType.TEXT,Data="===GuppOS Log===",Position=new Vector2(lcd.drawableSurface.Width/2,
logLineY)+lcd.drawableSurface.Position,Alignment=TextAlignment.CENTER,RotationOrScale=1.0f*lcd.fontScale,Color=titleColour,
FontId=basicFont});logLineY+=35*lcd.fontScale;for(int i=0;i<=5;i++){foreach(KeyValuePair<string,logItem>item in logItems){if(
item.Value.priority==i){StringBuilder logLine=new StringBuilder("("+item.Value.life+") "+item.Value.category+": "+item.Value
.content);float lineHeight=25*lcd.fontScale;if(logLine.Length>logLineMaxChar){int lineBreaks=logLine.Length/
logLineMaxChar;for(int l=1;l<=lineBreaks;l++){logLine.Insert(logLineMaxChar*l,"\n");lineHeight+=25*lcd.fontScale;}}frame.Add(new
MySprite{Type=SpriteType.TEXT,Data=logLine.ToString(),Position=new Vector2(0,logLineY)+lcd.drawableSurface.Position,Alignment=
TextAlignment.LEFT,RotationOrScale=0.8f*lcd.fontScale,Color=basicColour,FontId=basicFont});logLineY+=lineHeight;}}}}if(logAll)
logItems["LCDL02"]=new logItem{category="LCDs",content="Log LCD drawn.",priority=5,maxLife=2};}void DrawInfoLCD(GOSLCD lcd){lcd.
lcdScreen.ContentType=ContentType.SCRIPT;lcd.lcdScreen.Script="";using(var frame=lcd.lcdScreen.DrawFrame()){float spriteXPos=0;
float spriteYPos=0;Color spriteColour;string spriteText;spriteXPos=lcd.drawableSurface.Width/2;spriteYPos=0*lcd.fontScale;
spriteColour=titleColour;spriteText="===GuppOS Ship Status===";frame.Add(new MySprite{Type=SpriteType.TEXT,Data=spriteText,Position=
new Vector2(spriteXPos,spriteYPos)+lcd.drawableSurface.Position,Alignment=TextAlignment.CENTER,RotationOrScale=1.0f*lcd.
fontScale,Color=spriteColour,FontId=basicFont});spriteXPos+=0;spriteYPos+=30*lcd.fontScale;spriteColour=postureColour;spriteText=
$"--Posture: {currentPostureName}--";frame.Add(new MySprite{Type=SpriteType.TEXT,Data=spriteText,Position=new Vector2(spriteXPos,spriteYPos)+lcd.
drawableSurface.Position,Alignment=TextAlignment.CENTER,RotationOrScale=0.9f*lcd.fontScale,Color=spriteColour,FontId=basicFont});
spriteXPos=0;spriteYPos+=25*lcd.fontScale;spriteColour=titleColour;spriteText="Power Manager:";frame.Add(new MySprite{Type=
...
spriteYPos+=0*lcd.fontScale;if(powerManagerEnabled){spriteColour=enabledColour;spriteText="Enabled";}else{spriteColour=
...
FontId=basicFont});spriteXPos=lcd.drawableSurface.Width;spriteYPos+=5*lcd.fontScale;if(mainReactors.groupEnabled==true){
spriteColour=onlineColour;spriteText="Reactors Online";}else{spriteColour=offlineColour;spriteText="Reactors Offline";}frame.Add(new
...
=TextAlignment.RIGHT,RotationOrScale=0.6f*lcd.fontScale,Color=spriteColour,FontId=basicFont});spriteXPos=5;spriteYPos+=20
*lcd.fontScale;spriteColour=titleColour;spriteText="Batteries  | Total | Functional | Level | Mode";frame.Add(new
MySprite{Type=SpriteType.TEXT,Data=spriteText,Position=new Vector2(spriteXPos,spriteYPos)+lcd.drawableSurface.Position,Alignment
=TextAlignment.LEFT,RotationOrScale=0.6f*lcd.fontScale,Color=spriteColour,FontId=basicFont});spriteXPos+=0;spriteYPos+=15
*lcd.fontScale;if(mainBatteries.batteryGroupMode==ChargeMode.Recharge){spriteColour=rechargeModeColour;}else if(
mainBatteries.batteryGroupMode==ChargeMode.Discharge){spriteColour=dischargeModeColour;}else{spriteColour=autoModeColour;}spriteText=$" Main      | {CentreText(mainBatteries.groupTotBats.ToString(),5)} | {CentreText(mainBatteries.groupFuncBats.ToString(),10)} | {CentreText(mainBatteries.groupPowerLevel.ToString("F0")+"%",5)} | {mainBatteries.batteryGroupMode.ToString()}"
...
Contains("starboard")){light.SetValue("Color",Color.Green);light.Enabled=on;}}}}catch(Exception exc){logItems[logID[0]+"E05:"+
...
groupPowerLevel<lowPowerPercent){if(!currentPosture.overrideReactors){mainReactors.SetGroupEnabled(true);}}else if(mainBatteries.
groupPowerLevel>highPowerPercent){if(!currentPosture.overrideReactors){mainReactors.SetGroupEnabled(false);}}if(mainBatteries.
groupPowerLevel<1||mainBatteries.groupFuncBats<1){if(!currentPosture.overrideBatteries){backupBatteries.SetBatMode(ChargeMode.Auto);}}
else if(mainBatteries.groupPowerLevel>=lowPowerPercent){if(!currentPosture.overrideBatteries){backupBatteries.SetBatMode(
ChargeMode.Recharge);}}}static string mainDriveGroupName,rcsGroupName,gyroGroupName;DriveGroup mainDrives;DriveGroup rcsThrusters;
class DriveGroup:BlockGroup<IMyThrust>{public DriveGroup(Program program,string name):base(program,name,new string[]{"Pr",
"Propulsion"}){}public float maxThrust=0,maxEffectiveThrust=0;public override void ReadStep(){maxThrust=0;maxEffectiveThrust=0;
foreach(IMyThrust drive in thingList){if(drive.IsWorking){maxThrust+=drive.MaxThrust;maxEffectiveThrust+=drive.
MaxEffectiveThrust;}}}}GyroGroup mainGyros;class GyroGroup:BlockGroup<IMyGyro>{public GyroGroup(Program program,string name):base(program,
name,new string[]{"Pr","Propulsion"}){}}static string mainFuelTankGroupName,mainAirTankGroupName;GasTankGroup mainFuelTanks;
GasTankGroup mainAirTanks;class GasTankGroup:BlockGroup<IMyGasTank>{public GasTankGroup(Program program,string name):base(program,
name,new string[]{"Ta","Tanks"}){}public bool?stockpileOn=null;public float capacity=0,stored=0,lastStored=0,level=0,
flowRate=0;public int totalCount=0,funcCount=0;public override void ReadStep(){lastStored=stored;float secondsSince=(float)
program.timeSinceLooped.TotalSeconds;capacity=0;stored=0;totalCount=0;funcCount=0;foreach(IMyGasTank tank in thingList){
totalCount++;if(tank.IsFunctional){funcCount++;capacity+=tank.Capacity;stored+=Convert.ToSingle(tank.Capacity*tank.FilledRatio);}}
level=stored/capacity*100;if(secondsSince>0){flowRate=(lastStored-stored)/secondsSince;}}public void SetStockpile(bool
stockpile){try{if(stockpile!=stockpileOn){foreach(IMyGasTank tank in thingList){tank.Stockpile=stockpile;}stockpileOn=stockpile;}
}catch(Exception exc){logItems[logID[0]+"E04:"+groupName]=new logItem{category=logID[1],content=
"Error when setting stockpile mode of "+groupName+" -"+exc,priority=1,maxLife=3};}}}
